...
"""How You Could Actually Implement Your Idea
Approach 1: Manual Per-Layer Weight Zeroing (Quick Fix)
Use the existing layerwise scores but hard-zero adapters not selected for a given layer before passing to the model:
python# Per-layer selection based on top-k scores
per_layer_adapter_mask = {}
for layer_name, scores in layerwise_mapping_matrix.items():
    # Only keep top-2 adapters for THIS layer, zero out the rest
    top_k_idx = np.argsort(scores[0])[::-1][:2]
    mask = np.zeros_like(scores)
    mask[0, top_k_idx] = scores[0, top_k_idx]
    per_layer_adapter_mask[layer_name] = mask

# Layer 0 might pass: [0.71, 0.29, 0.0]  → A and B active
# Layer 1 might pass: [0.0,  0.45, 0.55] → B and C active
# Layer 2 might pass: [0.60, 0.0,  0.40] → A and C active
This is achievable within the current architecture since layerwise_mapping_tensor already goes per-layer — you're just enforcing sparsity more aggressively."""
